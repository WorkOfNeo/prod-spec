...
export function LogoImageList({ initialLogoImages }: { initialLogoImages: LogoImage[] }) {
  const router = useRouter();
  const [editing, setEditing] = useState<LogoImage | null>(null);
    ...
    <>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-zinc-500">
          Upload the logo artwork, then link it to a style from the style&apos;s edit page —{" "}
          <code className="font-mono">{"{{logo:custom}}"}</code> prints the linked logo.
        </span>
      </div>

      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
        {initialLogoImages.length === 0 ? (
          <div className="col-span-full rounded-lg border border-zinc-200 bg-white p-8 text-center text-sm text-zinc-500">
            No logos yet. Click <strong>+ New logo</strong> to upload one.
          </div>
        ) : (
          initialLogoImages.map((q) => (
            <LogoImageCard key={q.id} logoImage={q} onEdit={() => setEditing(q)} />
          ))
        )}
      </div>
...
function LogoImageCard({ logoImage, onEdit }: { logoImage: LogoImage; onEdit: () => void }) {
  const dataUrl = asDataUrl(logoImage.image);
  return (
    <div
      className={`rounded-lg border p-3 ${
        logoImage.active ? "border-zinc-200 bg-white" : "border-amber-300 bg-amber-50 opacity-70"
      }`}
    >
      <div className="flex h-24 items-center justify-center rounded-md bg-zinc-50">
        {dataUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={dataUrl} alt={logoImage.name} className="h-20 w-20 object-contain" />
        ) : (
      ...
      <div className="mt-2 flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-medium">{logoImage.name}</div>
          {!logoImage.active && <div className="text-xs text-amber-700">disabled</div>}
        </div>
        <button
          type="button"
          onClick={onEdit}
...
function LogoImageDialog({
  ...
  const [name, setName] = useState(logoImage?.name ?? "");
  const [image, setImage] = useState(logoImage?.image ?? "");
  const [active, setActive] = useState(logoImage?.active ?? true);
  ...
  async function save() {
    ...
    try {
      const url =
        mode === "create" ? "/api/admin/logo-images" : `/api/admin/logo-images/${logoImage!.id}`;
  ...
  async function destroy() {
    if (!logoImage) return;
    if (!confirm(`Delete "${logoImage.name}"? This is permanent — toggling Active off is safer.`))
      return;
    setBusy(true);
    setErr(null);
    try {
      const res = await fetch(`/api/admin/logo-images/${logoImage.id}`, { method: "DELETE" });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
      ...
      >
        ...
        <label className="block text-xs font-medium text-zinc-700">
          ...
          <input
            ...
            placeholder="e.g. Paw Patrol brand logo"
          />
        </label>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium text-zinc-700">
              Logo image (SVG, PNG, or JPG — drop anywhere in this dialog)
            </label>
            <input
              type="file"
