...
// service):
//
//   ?uploadOnly=1  — the RECURRING upload sweep (WS3). Railway cron hits this
//     every 15–30 min: reconcile the backlog (styles approved before queue
//     capture existed, above the supplierSendMinPo cutoff) into the queue,
//     then push every still-pending upload into the suppliers' own SharePoint
//     folders. NO email — files just land "on the go". Rows that used their
//     MAX_PUSH_ATTEMPTS strikes are left floated for the midnight sweep.
//     Records a CronRun (kind "supplier-upload") for /automation activity.
//
...
export async function POST(req: NextRequest) {
  ...
  if (uploadOnly) {
    ...
    return NextResponse.json({ ok: true, enabled, reconciled, ...sweep });
  }
