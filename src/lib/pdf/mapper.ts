...
import { parsePriceAmount } from "./price-parse";
...
export function parsePrice(raw: string): StyleData["price"] | undefined {
  ...
  // No currency is carried: layouts print their own currency text (the Coop
  // DR barcode tag's line is literally "{{price}} KR"), and the free-text
  // column's "KR" is ambiguous across DKK/NOK/SEK anyway. See price-parse.ts.
