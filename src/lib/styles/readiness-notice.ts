...
export type ReadinessStep = {
  ...
  // care, Composition" the way the design shows. `note` carries a per-output
  // explanation (the exclusion rule that skipped it) for the excluded step.
  outputs?: { name: string; fields: MissingDetailField[]; note?: string | null }[];
  owner?: StepOwner;
...
type NormalizedOutputs = {
  ...
  excluded: number;
  // The excluded outputs with the RULE that skipped each, so the notice can
  // tell reviewers WHICH outputs a doc-type rule skipped and WHY (e.g.
  // "Wash care label — Product group contains 'Socks'").
  excludedOutputs: { name: string; reason: string | null }[];
...
function normalizeFromCurrent(outputs: CurrentOutput[]): NormalizedOutputs {
  ...
  for (const docs of byBase.values()) {
    ...
    if (isExcluded(slotState)) {
      n.excluded += 1;
      const ex = docs.find((d) => isExcluded(d.state)) ?? docs[0];
      n.excludedOutputs.push({ name: ex.name, reason: ex.exclusionReason });
      continue;
    }
    n.total += 1;
    switch (slotState) {
...
function outputSteps(
  ...
  if (n.excluded > 0) {
    steps.push({
      ...
      title:
        n.excluded === 1
          ? "1 output not generated by rule"
          : `${n.excluded} outputs not generated by rule`,
      detail:
        "A document-type rule skips these for this product — counted out of the total, not a failure and nothing to review:",
      outputs: n.excludedOutputs.map((o) => ({
        name: o.name,
        fields: [],
        // The rule that fired, e.g. "Not generated — Product group contains
        // “Socks” (Wash care rule)". Falls back to a generic line if a legacy
        // asset carries no stored reason.
        note: o.reason ?? "Skipped by a document-type rule for this product.",
      })),
      owner: "SYSTEM",
    });
  }
