...
// ---------------------------------------------------------------------
// {{sizeRange}} prints the style's FULL size run even inside a per-size /
// per-EAN repetition — the range is a property of the STYLE ("sizes
// available for this product"), so every split file must read the same.
// {{size}} (the row's size) and {{sizes}} (the row's list) stay narrowed;
// these lock that split, plus the one row type that legitimately covers a
// GROUP of sizes: a per-carton row, which ranges over its own sizes.
// ---------------------------------------------------------------------
...
test("no repetition — the range spans the whole run", () => {
  assert.equal(range(makeStyle()), "86/92–122/128");
});

test("repeat per size: every file prints the full range, not its own size", () => {
  const reps = repetitionStyles(makeStyle(), "size");
  assert.equal(reps.length, 4);
  for (const rep of reps) assert.equal(range(rep), "86/92–122/128");
...
test("a per-carton row ranges over ITS sizes, not the whole run", () => {
  ...
  assert.equal(range(reps[1]), "122/128"); // single-size carton — no dash
});

test("a single-size style prints the bare label, never a dash", () => {
