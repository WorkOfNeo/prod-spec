...
- **Cron schedule**: add one Railway **cron service** that pokes both runners every minute. Inline triggering covers the happy path; the cron handles missed runs, dropped fire-and-forget kicks, and stuck-row retries. **Use the committed script, not a dashboard curl** — point the cron service at this repo, set its **Railway config file to `railway.cron.json`** (which skips the app build and runs `node scripts/cron-tick.mjs`), set a **Cron Schedule** (`* * * * *`), and give it `PROD_SPEC_BASE_URL` + `JOB_RUNNER_SECRET` env vars.

  The script POSTs `/api/po-eans/run?secret=…&sweep=1` then `/api/jobs/run?secret=…&sweep=1`, reading the URL + secret from `process.env` (no shell expansion / quoting / `curl`-entrypoint pitfalls — those silently broke earlier: `$VAR` unexpanded, the command echoed instead of run). It logs each call's status (never the secret), so the cron's own logs show `[cron] 200 /api/po-eans/run {…}`. EAN call first so any generation it enqueues is queued before the job drain. `&sweep=1` is what (a) records the run on `/automation` and (b) runs the EAN retry sweep + generation backlog sweep. Cron-origin generation never emails (the in-app review inbox still fills); rejected outputs are never auto-regenerated; a style that fails to render 3× floats for manual attention.

  **PO cutoff (scoping):** auto-scrape *and* the generation sweep only touch styles whose **PO is at/above a cutoff** (`Style.poSeq >= automationMinPo` — `poSeq` is the numeric part of the PO, e.g. `63144` for `C-PO63144`, via `parsePoNumberValue`). Set the cutoff on `/automation` by pasting a PO ("from this one onward"); unset = whole backlog. This is the right delimiter when everything was imported recently (a date window can't tell new orders from old). Below-cutoff POs are **parked** (counted on `/automation`), never auto-processed, but still scrape-able per-row from `/po-eans`. A style also queues event-driven when its PO is newly filled/changed at ingest. (Mirrors the `/styles` done-group PO cutoff.)

  **Watch it:** `/automation` shows recent cron runs (what each did), queue depths, and a **Run now** button. If the queue is full but no runs are listed, the cron isn't reaching the app — check the cron service's own logs for the `[cron] …` lines (`base=MISSING` ⇒ the env var isn't set; no `[cron]` line at all ⇒ it isn't running `scripts/cron-tick.mjs`, i.e. the config-file path is wrong).
