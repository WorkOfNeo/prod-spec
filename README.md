...
- **Cron schedule**: add one Railway cron service that pokes both runners every minute. Inline triggering covers the happy path; the cron handles missed runs, dropped fire-and-forget kicks, and stuck-row retries. Start command:
  ...
  **Recent-window (scoping):** auto-scrape *and* the generation sweep only touch styles whose **PO landed within the last N days** (`Style.eanQueuedAt`; N is set on `/automation`, default 30, `0` = whole backlog). A style queues when its PO is newly filled/changed (event-driven), so a bulk re-sync can't drag the historical backlog back in. Out-of-window POs are **parked** (counted on `/automation`), never auto-processed, but still scrape-able per-row from `/po-eans` or by widening the window.
